...
```
⚠ 외부 파일 3개를 불러오지 못했다 — 화면만 다르게 보이고 편집·저장은 정확하다   [폴더 연결]
```

**폴더 연결**을 누르고 그 문서가 든 폴더를 고르면 원래 모습대로 보인다.
...
```
저장하지 않은 변경이 있다
3개를 고쳤다. deck/appendix.html 로 옮기면 사라진다.

              [취소]  [버리고 계속]  [저장하고 계속]
```

**저장하고 계속**을 누르면 저장한 뒤에 옮긴다. 덮어쓸 수 없는 문서(드롭·zip)라면
버튼이 `사본 내려받고 계속` 으로 바뀐다 — 원본을 되쓸 자리가 없기 때문이다.
