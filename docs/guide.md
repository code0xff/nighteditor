...
| --- | --- |
| 초록 실선 테두리 | 고칠 수 있다 |
| 초록 테두리 + 옅은 초록 배경 | 지금 고치는 중이다 |
| 회색 점선 테두리, 금지 커서 | 잠겼다 — 누르면 이유가 뜬다 (§6) |
...
- **잠긴 이유** — 사유별 개수 (§6)
- **변경 n** — 고친 블록 목록. 항목마다 **고치기 전(취소선)** 과 **고친 뒤**를 함께 보여준다.
...
고친 내용은 저장할 때까지 브라우저 메모리에만 있다. 그래서 잃을 수 있는 두 순간에 막아준다.

- 탭을 닫거나 새로고침하면 브라우저가 **정말 나갈지 되묻는다**
- 다른 파일을 열려고 하면 **변경 개수를 보여주고 확인을 받는다**
