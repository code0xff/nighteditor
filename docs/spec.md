...
외부 CSS·JS·이미지를 따로 두고 참조하는 문서는 **프리뷰에서 그 자원이 빠진 채 보인다.**
`srcdoc` 문서의 기준 URL 은 원본 파일이 있던 자리가 아니라 이 앱이기 때문이다.
편집과 저장은 영향받지 않는다 — 언제나 원본 문자열만 만지므로 화면이 어떻게 보이든
diff 는 정확하다. 다만 빠진 자원이 레이아웃을 만드는 문서라면 무엇을 고치는지
알아보기 어려울 수 있다.
