...
몫이라 잃는 것이 없다. 버리는 것은 상태 갱신과 알림·busy 해제뿐이다.
...
겹친 둘 중 먼저 끝난 쪽이 이겨 버린다. 그래서 갈아 끼우기는 **시작할 때** 세대를
받아 두고, 읽기를 마쳤을 때 더 새 세대가 시작돼 있으면 설치하지 않고 물러난다.
물러나는 흐름이 놓아줄 수 있는 것은 **제가 만든 것뿐이다** — 화면이 쓰고 있는
자원(blob URL)을 놓아줄 권리는 그 문서를 세운(또는 세울) 흐름에 있다. 알림과
busy·replacing 해제도 최신 세대의 몫이다 — 물러나는 흐름이 끄면 아직 읽는 중인
화면의 잠금이 풀린다.
