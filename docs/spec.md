...
레퍼런스 파일의 `.codelabel` 28개를 포함한 `span` 70개가 두 번째 경우다.
...
레퍼런스 파일은 빈 곳 클릭으로 슬라이드를 넘기므로, 막으면 첫 장에서 벗어날 수 없어
나머지를 편집할 방법이 사라진다.

| 클릭 위치 | 편집 중 | 처리 |
|---|---|---|
| 블록 | 무관 | 편집 열기, 아티팩트로 전달 안 함 |
| 블록 밖 | 예 | 편집 종료로 소비, 아티팩트로 전달 안 함 |
| 블록 밖 | 아니오 | **아티팩트로 통과** |
...
- [ ] `.code` 안에 중첩 요소가 있어도 잠금이 자손까지 상속된다
- [ ] `&amp;amp;` 같은 이중 엔티티가 라이브 텍스트와 일치한다 (오탐 잠금 없음)
