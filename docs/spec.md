...
| 상태 | 표시 |
|---|---|
| 편집 가능 · 호버 | 초록 실선 외곽선, 커서 `text` |
| 편집 중 | 초록 실선 외곽선 + 옅은 초록 배경 |
| 잠김 · 호버 | 회색 **점선** 외곽선, 커서 `not-allowed` |
...
**IME 조합 중의 `Enter` 는 예외다.** 한글 조합을 확정하는 키이므로 편집을 닫지 않고
기본 동작도 막지 않는다. 막으면 조합이 확정되지 않아 글자를 완성할 수 없다.
