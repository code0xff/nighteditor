...
- 예약은 **사용자 행동의 순간**에 한다 — 훑기·대화상자·묻기·저장처럼 오래 걸리는
  일을 시작하기 **전에**. 늦게 받으면, 먼저 시작했지만 늦게 준비를 마친 흐름이 더
  새 예약을 받아 사용자의 마지막 선택을 덮는다
- 예약한 뒤의 모든 단계(훑기 → 묻기 → 저장 → 읽기 → 설치)는 단계를 마칠 때마다
  **자기 예약이 아직 최신인지** 한 자리에서 확인한다. 물음에 저장으로 답하고
  돌아온 흐름도, 폴더 훑기를 기다린 흐름도 같은 확인을 거친다
