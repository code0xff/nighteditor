...
src/
  ...
  preview/              ← iframe 내부에서 실행되는 에이전트 스크립트
    agent.ts            클릭 감지, contenteditable 토글, postMessage
    ime.ts              한글 조합 처리
  ui/                   ← React 컴포넌트
  store/                ← Zustand
```
...
**검증** `agent_payments.html`의 `<h1>` 12바이트 수정 → `diff` 결과 **1줄**, 나머지 110KB 바이트 동일.
...
**이유** 레퍼런스 파일 실측 — 원시 텍스트 노드 1,674개, **길이 중앙값 10자**.
`<b>` 때문에 한 문장이 3개 노드로 쪼개진다. 텍스트 노드 단위로 하면 사용자는 문장의 일부만 고칠 수 있고
인라인 태그 경계가 편집 장벽이 된다. 블록 단위로 묶으면 867개(`<title>` 포함)이고, 인라인 마크업은 DOM에 남아 자연히 보존된다.
...
**이유** 아티팩트 스크립트가 DOM을 재구성한다. 레퍼런스 파일의 `wrapSheets()`는 각 `.slide`의 자식 전체를
...
레퍼런스 파일의 `#cnt`, `.pg` 40개가 이 규칙으로 자동 걸러진다.
