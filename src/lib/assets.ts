 ...
 */
import { dirOf, rewriteCssUrls } from '@/core/assets';
...
export const EMPTY_BUNDLE: AssetBundle = { urls: new Map(), dispose: () => {} };
...
export async function buildAssets(files: ReadonlyMap<string, Blob>): Promise<AssetBundle> {
  ...
  for (const [path, blob] of files) {
    ...
    const css = rewriteCssUrls(await blob.text(), dirOf(path), (p) => urls.get(p));
